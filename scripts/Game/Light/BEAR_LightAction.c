...
class BEAR_LightAction : ScriptedUserAction
{
	...
	// As the BEAR_LaserPointerAction is on the Weapon_Base.et, make sure we have a light attached before showing the Action.
	// I don't like this solution because it will check constantly, even with no light attached...
	// A cleaner solution would be to have the BEAR_LightComponent add an Action to the weapon, but the engine does not support adding Actions.
	...
	{
		if(_isOn)
			outName = "Disable Light";
		else
			outName = "Enable Light";
		return true;
	}
	...
	override void PerformAction(IEntity pOwnerEntity, IEntity pUserEntity)
	{
		...
		foreach(Managed slot : slots)
		{
			...
			BEAR_LightComponent laser = BEAR_LightComponent.Cast(attachment.FindComponent(BEAR_LightComponent));
			if(laser) 
			{
				laser.SetLightEnabled(_isOn);
			}
		}
	}
}